...
namespace Terz_DataBaseLayer
{
    public class ReportCollection
    {
        ...
        public void LoadN(int limit,int rank,string keyword) {
            this.Reports = new List<Report>();
            Base.Init();
            var sql = "select * from report where ranque > "+rank+" and titulo like '"+keyword+ "' and privado <> '1' order by score DESC " + "LIMIT "+limit;
            MySqlDataReader myReader = Base.select(sql);
            while (myReader.Read())
            {
                Report report = new Report();
        ...
        public void LoadN(int limit,int rank,int categoryId,string keyword)
        {
            this.Reports = new List<Report>();
            Base.Init();
            var sql = "select * from report where ranque > " + rank+ " and titulo like '" + keyword + "' and id_categoria = " + categoryId+ " and privado <> '1' order by score DESC " + "LIMIT " + limit;
            MySqlDataReader myReader = Base.select(sql);
            while (myReader.Read())
            {
                Report report = new Report();
