var rdf;
function buildMapTimeLineChart(graph) {
    am4core.ready(function () {
        ...
        for (var i = 1; i < reducedDf.length; i++) {
            var cData = fixData(reducedDf[i][0]);
            var posData = world_timeline.map(function (e) { return e.date; }).indexOf(cData);

            var countryObj = {};
            countryObj["id"] = reducedDf[i][1];
            for (var j = 0; j < expressions.length; j++) {
                countryObj[expressions[j]] = parseFloat(reducedDf[i][j + 2]);
            }

            if (posData < 0) {
                var dataObj = { date: cData, list: [] };

                dataObj.list.push(countryObj);
                world_timeline.push(dataObj);
            }
            else {
                world_timeline[posData]["list"].push(countryObj);
            }
        }

        for (var i = 1; i < reducedDf.length; i++) {
            var cData = fixData(reducedDf[i][0]);
            var posData = total_timeline.map(function (e) { return e.date; }).indexOf(cData);
            if (posData < 0) {
                var dataObj = {};
                dataObj["date"] = cData;
                for (var j = 0; j < expressions.length; j++) {
                    dataObj[expressions[j]] = parseFloat(reducedDf[i][j + 2]);
                }
                total_timeline.push(dataObj);

            }
            else {
                for (var k = 0; k < expressions.length; k++) {
                    total_timeline[posData][expressions[k]] += parseFloat(reducedDf[i][k + 2]);
                }
            }
        }
        ...
        am4core.ready(function () {
            ...
            var colorList = [];
            var colorListDefault = [am4core.color("#ff8726"), am4core.color("#d21a1a"), am4core.color("#45d21a"), am4core.color("#1c5fe5")];
            ...
            colorList = colorList.concat(colorListDefault);
            ...
            for (var i = 0; i < expressions.length; i++) {
                ...
                colors[expressions[i]] = colorList[i];
            }
            ...
            for (var i = 0; i < mapData.length; i++) {
                var di = mapData[i];
                for (var j = 0; j < expressions.length; j++) {
                    if (di[expressions[j]] > max[expressions[j]]) {
                        max[expressions[j]] = di[expressions[j]];
                    }
                }
            ...
            var container = am4core.create("g" + graph.id, am4core.Container);
            ...
            for (var i = 0; i < expressions.length; i++) {
                series[expressions[i]] = addSeries(expressions[i], colorList[i]);
            }
            // add series
            ...
            function createColumnSeries() {
                ...
                for (var i = 0; i < expressions.length; i++) {
                    columnSeries[expressions[i]] = addSeries(expressions[i], colorList[i]);
                }
            ...
            for (var i = 0; i < expressions.length; i++) {
                buttons[expressions[i]] = addButton(expressions[i], colorList[i]);
            }
            ...
            function setCountryData(countryIndex) {
                ...
                for (var i = 0; i < lineChart.data.length; i++) {
                    ...
                    if (countryData) {
                        ...
                        for (var j = 0; j < expressions.length; j++) {
                            dataContext[expressions[j]] = countryData[expressions[j]];
                        }
            ...
            function showWorld() {
                ...
                for (var i = 0; i < lineChart.data.length; i++) {
                    ...
                    for (var j = 0; j < expressions.length; j++) {
                        dataContext[expressions[j]] = di[expressions[j]];
                    }
            ...
            function updateMapData(data) {
                ...
                bubbleSeries.dataItems.each(function (dataItem) {
                    for (var j = 0; j < expressions.length; j++) {
                        dataItem.dataContext[expressions[j]] = 0;
                    }
                ...
                for (var i = 0; i < data.length; i++) {
                    ...
                    if (image) {
                        ...
                        for (var j = 0; j < expressions.length; j++) {
                            image.dataItem.dataContext[expressions[j]] = di[expressions[j]];
                        }
    ...
    document.getElementById("g" + graph.id).style.backgroundColor = "#212327";
}